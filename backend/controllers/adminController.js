...
const getDashboardStats = async (req, res) => {
  try {
    const { filter = "week" } = req.query; // week, month, year
    ...
    // 2. DỮ LIỆU BIỂU ĐỒ THEO BỘ LỌC (DỮ LIỆU THẬT)
    let chartQuery = "";
    let dateFormat = "";
    ...
    if (filter === "week") {
      // Xem 7 ngày gần nhất
      chartQuery = `
                SELECT DATE_FORMAT(OrderDate, '%a') AS name, SUM(TotalAmount) AS revenue 
                FROM Orders 
                WHERE OrderDate >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND Status = 'Đã giao'
                GROUP BY DATE(OrderDate) ORDER BY DATE(OrderDate) ASC
            `;
    } else if (filter === "month") {
      // Xem 4 tuần trong tháng
      chartQuery = `
                SELECT CONCAT('Tuần ', CEIL(DAY(OrderDate)/7)) AS name, SUM(TotalAmount) AS revenue 
                FROM Orders 
                WHERE MONTH(OrderDate) = MONTH(CURDATE()) AND YEAR(OrderDate) = YEAR(CURDATE()) AND Status = 'Đã giao'
                GROUP BY name ORDER BY name ASC
            `;
    } else if (filter === "year") {
      // Xem 12 tháng
      chartQuery = `
                SELECT DATE_FORMAT(OrderDate, 'Tháng %c') AS name, SUM(TotalAmount) AS revenue 
                FROM Orders 
                WHERE YEAR(OrderDate) = YEAR(CURDATE()) AND Status = 'Đã giao'
                GROUP BY MONTH(OrderDate) ORDER BY MONTH(OrderDate) ASC
            `;
    }
...
const updateProductStock = async (req, res) => {
  try {
    ...
    await pool.query("UPDATE Products SET Stock = ? WHERE ProductID = ?", [
      stock,
      id,
    ]);
    res
...
const toggleProductStatus = async (req, res) => {
  try {
    ...
    await pool.query("UPDATE Products SET Status = ? WHERE ProductID = ?", [
      status,
      id,
    ]);
    res
  ...
  try {
    ...
    if (error.code === "ER_ROW_IS_REFERENCED_2") {
      return res.status(400).json({
        success: false,
        message:
          'Không thể xóa khách hàng này vì họ đã có lịch sử mua hàng. Hãy sử dụng chức năng "Khóa" thay thế!',
      });
    }
    res
      .status(500)
